...
--
-- Last modifications:
-- v2.0.5: added print message when reading file with invalid JSON format
...
local function readFile(dataGroup, useResourceDirectory, returnNilIfNotExist)
    ...
    if fh then
        ...
        local function readContent(value)
            local decodedDataString = decode(value)
            value = json.decode(decodedDataString)
            if decodedDataString and decodedDataString ~= "" and value == nil then
                print("[rb-storage] Data contents is with an invalid JSON format")
            end
            return value
      ...
      print("[rb-storage] Couldn't load storage: ", reason)
...
function storage.get(dataGroup, key, useResourceDirectory)
    ...
    if data == nil then -- data is not in memory, try to read from file
        data = readFile(dataGroup, useResourceDirectory or false, returnNilIfNotExist)
    end
